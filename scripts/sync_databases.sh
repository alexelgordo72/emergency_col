...
# ============================================================
# Emergency Col SGRD - Sincronizar Base de Datos
# ============================================================
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Sincronizar Base de Datos${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
ssh almofa@10.147.17.2 "docker exec db_sgrd pg_dump -U admin_comunidad -d comunidad_db --no-owner --no-acl" > /tmp/comunidad_db_temp.sql

if [ $? -ne 0 ]; then
    echo -e "${RED}❌ Error al exportar${NC}"
    exit 1
fi
...
sleep 10
ssh sgrd@192.168.194.1 "docker exec -i db_sgrd psql -U admin_comunidad -d comunidad_db" < /tmp/comunidad_db_temp.sql
ssh sgrd@192.168.194.1 "docker start api_sgrd"

echo -e "${GREEN}✅ Sincronización completada${NC}"

# Limpiar
rm -f /tmp/comunidad_db_temp.sql
