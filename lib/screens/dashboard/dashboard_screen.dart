...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  String zonaSeleccionada = 'Todos'; // 'Todos', 'Urbana', 'Rural'
  ...
  void _procesarDatos() {
    ...
    for (var r in filtered) {
      ...
      if (obs.contains('perro') || obs.contains('Perro')) {
        animales['Perros'] = (animales['Perros'] ?? 0) + 1;
      }
      if (obs.contains('gato') || obs.contains('Gato')) {
        animales['Gatos'] = (animales['Gatos'] ?? 0) + 1;
      }
      if (obs.contains('gallina') || obs.contains('Gallina')) {
        animales['Gallinas'] = (animales['Gallinas'] ?? 0) + 1;
      }
      if (!obs.contains('perro') && !obs.contains('gato') && !obs.contains('gallina') && obs.isNotEmpty) {
    ...
    // Obtener listas de comunas y barrios (para filtros)
    comunas = ['Todas'];
    for (var r in filtered) {
      if (r.comuna != null) {
  ...
  Widget _buildGraficos() {
    return Column(
      children: [
        // Prioridades
        _buildCard(
          '🎯 Prioridades de Visitados',
          _buildPrioridadesChart(),
        ),
        const SizedBox(height: 16),
