...
def actualizar_estado_reporte(reporte_id: str, update: dict):
    try:
        ...
        # ✅ ACTUALIZAR estado_actual en reportes_comunitarios
        cur.execute("""
            UPDATE reportes_comunitarios 
            SET estado_actual = %s, ultima_actualizacion = CURRENT_TIMESTAMP
            WHERE id = %s;
...
def eliminar_barrio(barrio_id: int):
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM reportes_comunitarios WHERE barrio_id = %s;", (barrio_id,))
        count = cur.fetchone()[0]
        if count > 0:
            cur.close()
            conn.close()
            raise HTTPException(status_code=400, detail=f"No se puede eliminar: {count} reportes asociados")
...
def actualizar_barrio(barrio_id: int, barrio: BarrioUpdate):
    try:
        ...
        # Verificar si el nombre ya existe en otro barrio
